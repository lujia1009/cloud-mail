...
旧版来源为 Git 提交 `ec7a2bb17c950576c38d7308128cac7696fea169` 的 `mail-vue/src`；新版为工作区 `mail-react/src`。先从旧版路由、视图、布局、store、请求封装和富文本组件提取功能，再与新版实际调用、后端 `mail-worker/src/entity/setting.js` 和 `setting-service.js` 对照。`cd mail-react && pnpm audit:compat` 可重新生成 [逐条 API、站点动态配置和后台设置对照表](mail-react/compat-api.generated.md)，包含旧版与新版文件行号及静态覆盖结果。下表是对 API 声明检查无法判断的交互、参数与保存语义进行的源码复核。
...
- 本轮根据源码对照修复了动态配置、权限、用户管理、全站邮件、统计、设置保存、富文本与发件状态等回归。**未修改 `mail-worker/src` 的业务逻辑。**
- 静态审计未留下已识别的旧 API、站点动态字段或后台设置字段缺口。真实服务下的角色权限组合、OAuth、Turnstile、R2/S3、邮件投递和原有 IndexedDB 数据仍须部署环境联调；当前环境没有这些服务的测试凭据，因此不将静态通过表述为运行时“完全兼容”。
